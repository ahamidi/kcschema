...
func TestPayload_DBZType(t *testing.T) {
	r := sampleRecordWithDBZSchema()
	p := Payload(r)

	pt := p.Type()
	if pt != DebeziumCDCType {
		t.Errorf("expected %s, got %s", DebeziumCDCType, pt)
	}
}

func TestPayload_DBZParse(t *testing.T) {
	r := sampleRecordWithDBZSchema()
	p := Payload(r)
...
func sampleRecordWithDBZSchema() []byte {
	return []byte(`{
        "payload": {
          "after": {
            "id": 9582724,
        ...
        "schema": {
          "fields": [
            {
              ...
              "name": "resource.public.User.Value",
              "optional": true,
              "type": "struct"
            },
            {
          ...
          "name": "resource.public.User.Envelope",
          "optional": false,
          "type": "struct"
        }
      }`)
